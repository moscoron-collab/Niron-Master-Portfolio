...
function addStatementEntry(data) {
  ...
  // Duplicate check: same period_start (col B) + same property (col C)
  var lastRow = sh.getLastRow();
  if (lastRow >= 2) {
    ...
    for (var i = 0; i < existing.length; i++) {
      ...
      if (ps === periodStart && String(existing[i][2]) === String(data.property)) {
  ...
  sh.getRange(nextRow, 1, 1, 12).setValues([[
    dateRange, periodStart, data.property,
    noi, 0,        // disbursement (= NOI), mgmt fee
    0, 0, 0,       // mortgage, tax/12, ins/12
    0, noi,        // maintenance, net cashflow
    'Manual Entry', loggedAt
  ]]);
  return ContentService.createTextOutput(JSON.stringify({ok: true, row: nextRow, message: 'Saved ' + data.property + ' ' + periodStart})).setMimeType(ContentService.MimeType.JSON);
}
