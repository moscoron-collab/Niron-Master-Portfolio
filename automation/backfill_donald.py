"""
AppFolio Historical Backfill — 5070 DONALD, LLC (per-property).
Paginates through AppFolio Owner Statements for the Donald card and, for every
historical Owner Packet, writes one row PER UNIT (5060..5082 E Donald Ave) to the
Niron sheet's "Property Detail" tab — mirroring run_donald.py (which it imports for
the parsing + fixed costs, so the two never drift apart).

This NEVER touches the History tab or the consolidated Donald row the dashboard cards
use; it only fills the isolated Property Detail tab. Safe to re-run — it dedups on
(property, month).

Trigger manually via GitHub Actions (backfill_donald.yml) with the months input.
"""
...
# Reuse the exact parser + fixed costs from the monthly script.
import run_donald as D
...
APPFOLIO_OWNER_NAME = D.APPFOLIO_OWNER_NAME    # "5070 Donald, LLC"
LLC_NAME      = D.LLC_NAME
DETAIL_TAB    = D.DETAIL_TAB
DETAIL_HEADER = D.DETAIL_HEADER
...
def build_row(pkt, r, now_str):
    fixed = D.DONALD_FIXED_COSTS.get(r["property"], {"mortgage": 0, "ins_mo": 0})
    return [
...
def main():
    print(f"Starting Donald per-property backfill — up to {MAX_MONTHS} months...")
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                for pkt in packets:
                    ...
                    try:
                        file_path = os.path.join(tmp_dir, f"donald_{month_label}{pkt['ext']}")
                        download_via_url(page, pkt["href"], file_path)
                        pdf_path = get_pdf_path(file_path, tmp_dir, f"donald_{month_label}")
                        if not pdf_path:
                            errors.append(f"{month_label}: Owner Packet.pdf not found")
                            continue
                        per_property = D.extract_per_property_from_pdf(pdf_path)
